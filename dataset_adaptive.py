    ...
    def __init__(
        ...
        for idx in tqdm(range(safe_start_idx, len(df))):
            ...
            for _ in range(self.seq_len):
                ...
                emo_step = step
                emo_start_idx = max(0, emo_end_idx - emo_step + 1)
                emotion_window = df.loc[emo_start_idx:emo_end_idx]

                emotion_features = []
                for col in self.emotion_base_cols:
                    emotion_features.append(float(emotion_window[col].mean()))
                    emotion_features.append(float(emotion_window[col].iloc[-1]))
